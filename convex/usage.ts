...
export const recordUsage = mutationGeneric({
  args: { serviceKey: v.string(), legacyUserId: v.string(), inputTokens: v.number(), outputTokens: v.number() },
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    if (existing) {
      await ctx.db.patch(existing._id, { requests: existing.requests + 1, inputTokens: existing.inputTokens + inputTokens, outputTokens: existing.outputTokens + outputTokens, updatedAt: now });
      return existing._id;
    }
    return ctx.db.insert("usageLedger", { userId: user._id, usageDate, requests: 1, inputTokens, outputTokens, createdAt: now, updatedAt: now });
  },
});
